...
public interface JobRepository extends JpaRepository<Job, Long> {
       ...
       @Query("SELECT j FROM Job j WHERE j.isActive = true AND j.status = 'OPEN' AND " +
                     ...
                     "(:location IS NULL OR :location = '' OR LOWER(j.location) LIKE LOWER(CONCAT('%', :location, '%')) OR "
                     +
                     "LOWER(j.city) LIKE LOWER(CONCAT('%', :location, '%'))) AND " +
                     "(:jobType IS NULL OR j.jobType = :jobType) AND " +
                     "(:workMode IS NULL OR j.workMode = :workMode) AND " +
                     "(:category IS NULL OR :category = '' OR :category = 'All' OR LOWER(j.category) = LOWER(:category))")
       ...
       @Query("SELECT j.category, COUNT(j) FROM Job j WHERE j.isActive = true AND j.status = 'OPEN' GROUP BY j.category")
       List<Object[]> getJobCountsByCategory();

       Long countByCompanyIdAndStatus(Long companyId, Job.JobStatus status);

       Long countByStatus(Job.JobStatus status);

       Long countByWorkModeAndStatus(Job.WorkMode workMode, Job.JobStatus status);
}
